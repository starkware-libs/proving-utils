    ...
    pub fn get_plain_fact_topologies(
        ...
        if self.subtasks.len()
            != n_tasks
                .to_biguint()
                .to_usize()
                .ok_or("Failed to convert Felt252 to usize")?
        {
            return Err(format!(
        ...
        match helper.output_type.as_str() {
            "CompositePackedOutput" => {
                ...
                let outputs = helper
                    ...
                    .into_iter()
                    .map(|x| {
                        BigUint::parse_bytes(x.to_string().as_bytes(), 10)
                            .ok_or(D::Error::custom("Failed to parse number"))
                            .unwrap()
                            .into()
                    })
                    .collect();
...
struct TaskSpecHelper {
    ...
    task_type: String,
    use_poseidon: bool,
    path: Option<PathBuf>,        // Optional path
    program: Option<Value>,       // Optional program data
    program_input: Option<Value>, // Optional program input
}
...
pub struct ExtractedProofValues {
    ...
    pub n_queries: u64,
    pub fri_witnesses_leaves: Vec<Vec<BigUint>>,
    pub fri_witnesses_authentications: Vec<Vec<BigUint>>,
}

#[derive(Debug)]
pub struct ExtractedIDsAndInputValues {
    pub log_trace_domain_size: u32,
    pub log_eval_domain_size: u32,
    pub layer_log_sizes: Vec<u32>,
    pub num_columns_first: BigUint,
    pub num_columns_second: BigUint,
    pub constraint_degree: BigUint,
}

// Struct needed for deserialization of the public input. Owed version of PublicInput.
