...
/// ec_op_auto_deduction_rule_wrapper ids.new_ec_op_ptr = segments.add()
/// ec_op_seg_idx = ids.new_ec_op_ptr.segment_index
/// ec_op_cache = {}
/// vm_add_auto_deduction_rule(ec_op_seg_idx,
/// ec_op_auto_deduction_rule_wrapper(ec_op_cache=ec_op_cache))
...
pub fn simulate_ec_op_fill_mem_with_bits_of_m(
    ...
    (0..m_max_bits.to_usize().unwrap()).try_for_each(|i| {
        let bit = MaybeRelocatable::Int((curr_m.clone() % 2u32).into());
        curr_m >>= 1;
