...
pub fn finalize_fact_topologies_and_pages(
    ...
    let mut aggregator_fact_topologies: Vec<FactTopology> =
        exec_scopes.get(vars::AGGREGATOR_FACT_TOPOLOGIES)?;
    assert_eq!(
        aggregator_fact_topologies.len(),
        1,
        "Expected exactly one fact topology for the aggregator task"
    );
    let aggregator_fact_topology = aggregator_fact_topologies.remove(0);
