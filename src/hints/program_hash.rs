...
pub enum ProgramHashError {
    ...
    /// Conversion from Felt252 to FieldElement failed. This is unlikely to
    /// happen unless the implementation of Felt252 changes and this code is
...
///     assert len(data) >= 1, f"len(data) for hash chain computation must be >=
/// 1; got: {len(data)}."     return functools.reduce(lambda x, y: hash_func(y,
/// x), data[::-1])
...
///
/// Converts the builtin name to bytes then attempts to create a field element
/// from these bytes. This function will fail if the builtin name is over 31
/// characters.
...
/// This function converts a `Felt252` to a `FieldElement` using a safe, albeit
/// inefficient, method.
...
///     builtin_list = [from_bytes(builtin.encode("ascii")) for builtin in
/// program.builtins]     # The program header below is missing the data length,
/// which is later added to the data_chain.     program_header =
/// [bootloader_version, program.main, len(program.builtins)] + builtin_list
...
pub fn compute_program_hash_chain(
    ...
    let builtin_list: Result<Vec<FieldElement>, _> =
        program.builtins.iter().map(builtin_to_field_element).collect();
    ...
    let program_data: Result<Vec<_>, _> =
        program.data.iter().map(maybe_relocatable_to_field_element).collect();
    ...
    let hash = if use_poseidon {
        let data: Vec<FieldElement> =
            data_chain[1..].iter().flat_map(|&v| v.iter().copied()).collect();
