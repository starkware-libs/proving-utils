...
pub fn load_and_parse_proof(
    ...
    // Retrieve proof data from execution scopes
    let proof: HashMap<String, JsonValue> =
        exec_scopes.get(VERIFIER_PROOF_INPUT).map_err(|_| {
            HintError::CustomHint("Failed to retrieve proof data from execution scopes.".into())
        })?;

    // Retrieve verifier identifiers from execution scopes
    let verifier_identifiers = get_program_identifies(exec_scopes, VERIFIER_PROGRAM)?;

    // Extract the "public_input" section from proof
    let public_input_json = proof
        .get("public_input")
    ...
    let extracted_proof_vals: ExtractedProofValues = extract_proof_values(&proof)?;
