...
fn check_circuit_verifier_configs() {
    ...
    assert_eq!(
        ...
        get_recursive_circuit_config().output_addresses
    );
    assert_eq!(
        circuit_config.n_blake_gates,
        get_recursive_circuit_config().n_blake_gates
    );
    assert_eq!(
        circuit_config.preprocessed_column_ids.len(),
        get_recursive_circuit_config().preprocessed_column_ids.len()
    );
    assert_eq!(
        circuit_config.preprocessed_root,
        get_recursive_circuit_config().preprocessed_root
    );
    assert_eq!(circuit_config.config, get_recursive_circuit_config().config);
