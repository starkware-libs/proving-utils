...
use crate::consts::{
    CIRCUIT_FRI_CONFIG, PRIVACY_CAIRO_VERIFIER_CONSTS_HASH,
    PRIVACY_RECURSION_CIRCUIT_PREPROCESSED_ROOT, PRIVACY_TRANSACTION_COMPONENTS,
};
use crate::{
    build_cairo_verifier_circuit, get_cairo_verifier_config, get_circuit_config,
    get_preprocessed_circuit, get_proof_config,
};
...
fn check_circuit_verifier_configs() {
    let cairo_verifier_config = get_cairo_verifier_config().unwrap();
    let preprocessed_circuit = get_preprocessed_circuit(&cairo_verifier_config);
    ...
    let _circuit_config = CircuitConfig {
    ...
    let _proof_config = ProofConfig::from_components(
    ...
    // TODO(AnatG): Uncomment these when stwo version is updated in stwo-circuits.
    // assert_eq!(circuit_config, get_circuit_config());
