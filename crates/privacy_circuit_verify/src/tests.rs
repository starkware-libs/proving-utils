...
fn check_circuit_verifier_configs() {
    ...
    // TODO: Change to separate asserts, use expect for updating preprocessed columns ids
    assert_eq!(circuit_config, get_recursive_circuit_config());

    let preprocessed_column_ids = preprocessed_circuit
        .preprocessed_trace
        .ids()
        .into_iter()
        .map(|id| id.id)
        .collect::<Vec<_>>();
    let preprocessed_column_ids: Vec<&str> = preprocessed_column_ids
        .iter()
        .map(|id| id.as_str())
        .collect();
    assert_eq!(
