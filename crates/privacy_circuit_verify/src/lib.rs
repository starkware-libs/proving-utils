...
pub fn verify(proof: &[u32], output_preimage: &[Felt]) -> Result<(), Box<dyn Error>> {
    ...
    let (public_claim, mut proof) = proof.split_at(PUBLIC_DATA_LEN + NUM_OUTPUTS + program_len);
    let proof_config = get_proof_config();
    let proof = deserialize_proof_with_config(&mut proof, &proof_config)?;
