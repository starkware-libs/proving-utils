...
static PROGRAM: LazyLock<Program> = LazyLock::new(|| {
    let bytes = include_bytes!("../../cairo_files/math/math_compiled.json");
...
fn test_assert_not_zero(#[case] value: Option<BigUint>, #[case] check: VmCheck<()>) {
    ...
    let res = runner.run_default("assert_not_zero", &args);
    check(&res);
}
...
fn test_assert_not_equal(
    ...
    let res = runner.run_default("assert_not_equal", &args);
    check(&res);
}
...
#[ignore = "Cairo VM returns Memory::RangeCheckNumOutOfBounds instead of HintError::ValueOutside250BitRange"]
// Case: value=2^250
// Expected: Error.
...
fn test_assert_250_bit(
    ...
    let res = runner.run_default("assert_250_bit", &args);
    check(&res);

    // If successful, verify the return value
    if res.is_ok() {
        let ret = runner.get_return_values(1).unwrap();
        assert_eq!(ret[0], add_maybe_relocatable(rc_base, 3));
    }
}
...
fn test_split_felt(mut runner: CairoFunctionRunner<'static>, #[case] idx: usize) {
    ...
    let expected_high = value >> 128;
    let expected_low = value & &mask_128;

    let args = cairo_args!(rc_base, value);
    runner
        .run_default("split_felt", &args)
    ...
    assert_eq!(
        ret[0],
        add_maybe_relocatable(rc_base, 3),
        "range_check_ptr mismatch for value {value}"
    );
    assert_eq!(
        ret[1],
        mr_from_biguint(&expected_high),
        "high mismatch for value {value}"
    );
    assert_eq!(
        ret[2],
        mr_from_biguint(&expected_low),
        "low mismatch for value {value}"
    );
}
...
fn test_assert_le_felt(
    ...
    if value0 <= value1 {
        runner
            .run_default("assert_le_felt", &args)
            .unwrap_or_else(|e| panic!("assert_le_felt failed for {value0} <= {value1}: {e}"));
        let ret = runner.get_return_values(1).unwrap();
        assert_eq!(
            ret[0],
            add_maybe_relocatable(rc_base, 4),
            "range_check_ptr mismatch for {value0} <= {value1}"
        );
    } else {
        let result = runner.run_default("assert_le_felt", &args);
...
fn test_assert_lt_felt(
    ...
    if value0 < value1 {
        runner
            .run_default("assert_lt_felt", &args)
        ...
        let result = runner.run_default("assert_lt_felt", &args);
...
fn test_abs_value(
    ...
    let args = cairo_args!(rc_base, value_case);
    let result = runner.run_default("abs_value", &args);
    check(&result);
    let abs_value = value_case.magnitude();
    if abs_value < &rc_bound_biguint {
        let ret = runner.get_return_values(2).unwrap();
        assert_eq!(ret[0], add_maybe_relocatable(rc_base, 1));
        assert_eq!(ret[1], mr_from_biguint(abs_value));
    }
}
...
fn test_sign(
    ...
    let args = cairo_args!(rc_base, value_case);
    let result = runner.run_default("sign", &args);
    check(&result);
    let abs_value = value_case.magnitude();
    if abs_value < &rc_bound_biguint {
        let ret = runner.get_return_values(2).unwrap();
        ...
        let expected_rc_ptr = if value_case.is_zero() {
            ...
            add_maybe_relocatable(rc_base, 1)
        };
        assert_eq!(ret[0], expected_rc_ptr);
        ...
        assert_eq!(ret[1], mr_from_biguint(&expected_sign));
    }
}
...
fn test_unsigned_div_rem(
    ...
    let args = cairo_args!(rc_base, value, div);
    let result = runner.run_default("unsigned_div_rem", &args);
    check(&result);
    ...
    if result.is_ok() {
        ...
        assert_eq!(ret[1], mr_from_biguint(&q), "quotient mismatch");
        assert_eq!(ret[2], mr_from_biguint(&r), "remainder mismatch");
    }
}
...
fn test_signed_div_rem(
    ...
    let args = cairo_args!(rc_base, value, div, bound);
    let result = runner.run_default("signed_div_rem", &args);
    check(&result);
    ...
    if result.is_ok() {
        ...
        assert_eq!(*rc_ptr, add_maybe_relocatable(rc_base, 4));
        // Expected_q = q % PRIME (field element conversion).
        let expected_q = MaybeRelocatable::from(Felt252::from(&q));
        assert_eq!(*result_q, expected_q);
        assert_eq!(*result_r, mr_from_biguint(&r));
    }
}
...
#[ignore = "Cairo VM returns HintError::AssertNNValueOutOfRange instead of HintError::SplitIntLimbOutOfRange"]
...
fn test_split_int(
    ...
    let args = cairo_args!(rc_base, value, n, base, bound, output_mr);
    let result = runner.run_default("split_int", &args);
    check(&result);
    ...
    if result.is_ok() {
        ...
        let ret = runner.get_return_values(1).unwrap();
        assert_eq!(ret[0], add_maybe_relocatable(rc_base, 2 * n as usize));
...
// Expected: Success.
#[case::random(None, expect_ok)]
#[ignore = "Cairo VM returns HintError::AssertNNValueOutOfRange instead of HintError::ValueOutside250BitRange"]
// Case: value=2^250
// Expected: Error.
...
fn test_sqrt(
    ...
    let args = cairo_args!(rc_base, value);
    let result = runner.run_default("sqrt", &args);
    check(&result);
...
fn test_horner_eval(mut runner: CairoFunctionRunner<'static>, #[case] n: usize) {
    ...
    let coefficients: Vec<BigUint> = (0..n)
        ...
        .collect();
    let coeff_mr: Vec<MaybeRelocatable> = coefficients.iter().map(mr_from_biguint).collect();
    ...
    let args = cairo_args!(n, coeff_mr, point);
    runner.run_default("horner_eval", &args).unwrap();
    ...
    assert_eq!(ret[0], mr_from_biguint(&expected));
}
...
fn test_is_quad_residue(mut runner: CairoFunctionRunner<'static>, #[case] x: Option<BigUint>) {
    ...
    let args = cairo_args!(x);
    runner.run_default("is_quad_residue", &args).unwrap();
    let ret = runner.get_return_values(1).unwrap();

    let expected = if is_quad_residue(&x, prime) {
        1i64
    } else {
        0i64
    };
