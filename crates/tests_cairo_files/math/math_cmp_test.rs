...
static PROGRAM: LazyLock<Program> = LazyLock::new(|| {
    let bytes = include_bytes!("../../cairo_files/math/math_cmp_compiled.json");
...
fn test_is_not_zero(
    ...
    runner.run_default("is_not_zero", &args).unwrap();
    let ret = runner.get_return_values(1).unwrap();
    assert_eq!(ret[0], mr_from_i64(expected_res));
}
...
fn test_is_le_felt(
    ...
    runner.run_default("is_le_felt", &args).unwrap();
    let ret = runner.get_return_values(2).unwrap();
    assert_eq!(ret[1], mr_from_i64(expected_res));
}
...
fn test_is_nn(mut runner: CairoFunctionRunner<'static>, #[case] value: BigUint) {
    ...
    runner.run_default("is_nn", &args).unwrap();
    let ret = runner.get_return_values(2).unwrap();
    assert_eq!(ret[1], mr_from_i64(expected_res));
}
...
#[case::pair_5(5)]
fn test_is_le(mut runner: CairoFunctionRunner<'static>, #[case] pair_idx: usize) {
    ...
    let args = cairo_args!(rc_base, value0, value1);
    runner.run_default("is_le", &args).unwrap();
    let ret = runner.get_return_values(2).unwrap();
    assert_eq!(ret[1], mr_from_i64(expected_res));
}
...
// Expected: returns 0 (values >= RC_BOUND).
#[case::pair_5(5)]
fn test_is_nn_le(mut runner: CairoFunctionRunner<'static>, #[case] pair_idx: usize) {
    let (value0, value1) = &PAIR_EXAMPLES[pair_idx];
    ...
    let args = cairo_args!(rc_base, value0, value1);
    runner.run_default("is_nn_le", &args).unwrap();
    let ret = runner.get_return_values(2).unwrap();
    assert_eq!(ret[1], mr_from_i64(expected_res));
}
...
fn test_is_in_range(
    ...
    runner.run_default("is_in_range", &args).unwrap();
    let ret = runner.get_return_values(2).unwrap();
    assert_eq!(ret[1], mr_from_i64(expected_res));
}
