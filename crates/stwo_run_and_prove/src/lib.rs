...
pub fn stwo_run_and_prove_from_file_path(
    ...
    let _span = span!(Level::INFO, "stwo_run_and_prove_from_file_path").entered();
...
mod tests {
    ...
    fn prepare_prove_config() -> (ProveConfig, PathBuf, TempPath, TempPath, TempDir) {
        ...
        (
            prove_config,
            program_output_tempfile.to_path_buf(),
    ...
    fn run_with_successful_mock_prover() -> (TempPath, TempPath) {
        let (prove_config, program_output, program_output_tempfile, proof_tempfile, _) =
            prepare_prove_config();
        ...
        stwo_run_and_prove_from_file_path(
            get_path(PROGRAM_FILE_NAME),
            None,
            Some(program_output),
            prove_config,
            mock_prover,
    ...
    fn run_with_failed_mock_prover() -> (TempPath, TempPath, TempDir) {
        ...
        let result = stwo_run_and_prove_from_file_path(
            get_path(PROGRAM_FILE_NAME),
            None,
            Some(program_output),
            prove_config,
            mock_prover,
            Some(debug_data_tempdir.path().to_path_buf()),
            false,
        );
        assert!(
