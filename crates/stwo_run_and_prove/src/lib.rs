...
mod tests {
    ...
    fn prepare_test_config() -> (TestConfig, TempPath, TempPath, TempDir) {
    ...
    fn run_stwo_run_and_prove_test(
        config: TestConfig,
        prover: Box<dyn ProverTrait>,
    ) -> Result<(), StwoRunAndProveError> {
        stwo_run_and_prove(
            ...
            config.program_output,
            config.prove_config,
            prover,
            config.debug_data_dir,
            config.save_debug_data,
        )
    }

    fn run_with_successful_mock_prover() -> (TempPath, TempPath) {
        let (config, program_output_tempfile, proof_tempfile, _) = prepare_test_config();
        ...
        run_stwo_run_and_prove_test(config, mock_prover)
    ...
    fn run_with_failed_mock_prover() -> (TempPath, TempPath, TempDir) {
        let (config, program_output_tempfile, proof_tempfile, debug_data_tempdir) =
            prepare_test_config();
        ...
        let result = run_stwo_run_and_prove_test(config, mock_prover);
        assert!(
