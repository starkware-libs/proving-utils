...
/// If `program_output` is provided, saves the program output to that path.
pub fn stwo_run_and_prove_inner(
    program: Program,
...
mod tests {
    ...
    fn prepare_prove_config() -> (ProveConfig, PathBuf, TempPath, TempPath) {
    ...
    fn run_with_successful_mock_prover() -> (TempPath, TempPath) {
        let (prove_config, program_output, program_output_tempfile, proof_tempfile) =
