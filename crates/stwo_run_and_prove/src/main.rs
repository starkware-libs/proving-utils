...
fn stwo_run_and_prove(
    ...
    let (successful_proof_attempt, output_vec) =
        prove_with_retries(prover_input, prove_config, entry_point)?;
...
mod tests {
    ...
    fn tests_helper_with_mock_prover() -> (TempPath, PathBuf, TempDir, PathBuf) {
        ...
        mock_prover
            ...
            .times(1)
            .returning(
                move |_cairo_prover_inputs, _proof_file, _format, _hash, _verify| {
        ...
        let successful_proof_attempt = result.unwrap();
        assert_eq!(
