...
mod tests {
    ...
    fn run_with_mock_prover_succeeds_on_retry(n_proof_attempts: usize) -> (TempPath, TempDir) {
        ...
        let mut results_vec: Vec<Result<Vec<[u32; 8]>, StwoRunAndProveError>> = Vec::new();
        for _ in 1..n_proof_attempts {
            results_vec.push(Err(StwoRunAndProveError::Verification)); // first n-1 calls
        }
        results_vec.push(Ok(vec![ARRAY_SUM_EXPECTED_OUTPUT])); // last call
        let mut results = results_vec.into_iter();
    ...
    fn test_stwo_run_and_prove_succeeds_on_retry(#[case] n_proof_attempts: usize) {
        ...
        for i in 1..n_proof_attempts + 1 {
            ...
            assert!(
                proof_file.exists(),
                "Proof file {:?} should exist after a run that succeeds on the second proof and verify try",
                i,
            );
        }
