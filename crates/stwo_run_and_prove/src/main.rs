...
) -> Result<u16, StwoRunAndProveError> {
...
fn prove_with_retries(
    ...
    prove_config: ProveConfig,
    prover: Box<dyn ProverTrait>,
) -> Result<(u16, OutputVec), StwoRunAndProveError> {
    ...
    for i in 0..prove_config.n_proof_attempts {
        ...
        let proof_file_path = prove_config.proofs_dir.join(format!("proof_{}", i + 1));
...
mod tests {
    ...
    fn tests_helper_with_mock_prover(n_proof_attempts: usize) -> (TempPath, TempDir) {
        let (args, program_output_tempfile, proofs_tempdir) = prepare_args(n_proof_attempts as u16);
        ...
        assert_eq!(
            successful_proof_attempt, n_proof_attempts as u16,
    ...
    fn tests_helper_with_verifier_failures(n_proof_attempts: usize) -> (TempPath, TempDir) {
        let (args, program_output_tempfile, proofs_tempdir) = prepare_args(n_proof_attempts as u16);
    ...
    fn test_stwo_run_and_prove_proof() {
        let (_, proofs_temp_dir) = tests_helper_with_mock_prover(1);
    ...
    fn test_stwo_run_and_prove_output() {
        let (output_temp_file, _) = tests_helper_with_mock_prover(1);
        let output_content =
    ...
    fn test_stwo_run_and_prove_retries() {
        let (output_temp_file, proofs_temp_dir) = tests_helper_with_verifier_failures(3);
        let proofs_dir = proofs_temp_dir.path().to_path_buf();

        for i in 0..3 {
            let proof_file = proofs_dir.join(format!("proof_{}", i + 1));
            assert!(
                proof_file.exists(),
