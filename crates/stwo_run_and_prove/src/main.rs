...
fn main() -> Result<(), StwoRunAndProveError> {
    ...
    let prove_args = ProveArgs {
        verify: args.verify,
        proofs_dir_path: args.proofs_dir_path,
    ...
    <MC::H as MerkleHasher>::Hash: CairoSerialize,
{
    ...
    std::fs::create_dir_all(&prove_args.proofs_dir_path)?; // create the directory if it doesn't exist

    for i in 0..prove_args.n_proof_attempts {
        let proof = prove_cairo::<MC>(prover_input.clone(), pcs_config, preprocessed_trace)?;
        let proof_file_path = prove_args.proofs_dir_path.join(format!("proof_{}", i));
