...
//! `0xDEAD`. The hash of each "all-dummy subtree at depth d" is precomputed off-circuit (see
//! [`dummy_hash_at_depth`]) and pinned in-circuit with `Eq` gates — there is no recursion into
...
/// Computes the canonical hash of an "all-dummy subtree" at the given depth, off-circuit.
///
/// Returned as a `HashValue<QM31>`. The unpacker uses these constants at every dummy-child
/// position in the preimage walk: the prover's guess for that position is pinned to this
/// constant via [`Eq`](circuits::ops::eq), and no Blake gate is spent on the dummy side.
///
/// # Recurrence
///
/// ```text
///   dummy_hash_at_depth(0)  = blake( pp_root.0 || pp_root.1 || DUMMY_LEAF_MARKER )
///   dummy_hash_at_depth(d)  = blake( pp_root.0 || pp_root.1
///                                  || dummy_hash_at_depth(d-1).0 || .1
///                                  || dummy_hash_at_depth(d-1).0 || .1 )
/// ```
///
/// Both children of an all-dummy subtree are the same — so the preimage at depth `d > 0` has the
/// child hash doubled.
pub fn dummy_hash_at_depth(_pp_root: HashValue<QM31>, _depth: u32) -> HashValue<QM31> {
    // Implementation arrives with the first test.
    todo!()
}
...
///
/// Dummy positions are pinned to [`dummy_hash_at_depth`] constants via `Eq` gates and cannot be
