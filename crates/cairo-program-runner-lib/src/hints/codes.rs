...
pub const CONCAT_AGGREGATOR_PARSE_TASK: &str = "def parse_bootloader_tasks_outputs(output):
    \"\"\"
...
pub const CONCAT_AGGREGATOR_GET_HANDLE_TASK_OUTPUT_WITH_SIZE: &str =
...
pub const CONCAT_AGGREGATOR_GET_HANDLE_TASK_OUTPUT_WITHOUT_SIZE: &str =
    "task_index = len(tasks_outputs) - ids.n_tasks
segments.load_data(ptr=ids.output_ptr, data=tasks_outputs[task_index])
