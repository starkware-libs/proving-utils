...
use crate::hints::utils::output_builtins_set_pages_by_size_and_fact_topology;
    ...
    fn execute_hint(
        ...
        match hint_data.code.as_str() {
            ...
            CONCAT_AGGREGATOR_SET_PAGES_AND_FACT_TOPOLOGY => {
                output_builtins_set_pages_by_size_and_fact_topology(vm, ids_data, ap_tracking, 10)
            }
            BUILTIN_USAGE_SET_MAX_SIZE_PAGES_AND_FACT_TOPOLOGY => {
                output_builtins_set_pages_by_size_and_fact_topology(vm, ids_data, ap_tracking, 3800)
            }
