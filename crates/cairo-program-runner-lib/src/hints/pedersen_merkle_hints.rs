...
mod tests {
    ...
    fn test_pedersen_merkle_load_input(pedersen_merkle_input: String) {
        ...
        let ids_data = prepare_ids_data_for_test(&["output"]);
        ...
        let expected_pedersen_merkle_input: PedersenMerkleInput =
            serde_json::from_str(&pedersen_merkle_input).unwrap(); // assert that output values are correctly set
        assert_eq!(
            vm.get_integer(Relocatable::from((2, 0))).unwrap().as_ref(),
            &Felt252::from(expected_pedersen_merkle_input.height)
        );
        assert_eq!(
            vm.get_integer(Relocatable::from((2, 1))).unwrap().as_ref(),
            &Felt252::from(expected_pedersen_merkle_input.prev_leaf)
        );
        assert_eq!(
            vm.get_integer(Relocatable::from((2, 2))).unwrap().as_ref(),
            &Felt252::from(expected_pedersen_merkle_input.new_leaf)
        );
        assert_eq!(
            vm.get_integer(Relocatable::from((2, 3))).unwrap().as_ref(),
            &Felt252::from(expected_pedersen_merkle_input.node_index)
        );
    ...
    fn test_pedersen_merkle_verify_auth_path_len() {
        ...
        assert_eq!(
            ...
            "Hint Error: Got too many values in auth_path."
        );
    }
    ...
    fn test_pedersen_merkle_idx_parity_to_ap(#[case] index: usize) {
        let mut vm = VirtualMachine::new(false, false);
        let ids_data = prepare_ids_data_for_test(&["index"]);
        let ap_tracking = ApTracking::new();
    ...
    fn test_pedersen_merkle_update(#[case] is_left_sibling: bool) {
        let mut vm = VirtualMachine::new(false, false);
        let mut exec_scopes = ExecutionScopes::new();
        let ids_data = prepare_ids_data_for_test(&["prev_node_hash", "new_node_hash"]);
        let ap_tracking = ApTracking::new();
