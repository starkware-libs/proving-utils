...
pub fn circuit_unpack_set_root_index(
    ...
    let node_root: [u32; BLAKE2S_DIGEST_N_WORDS] = match get_node(exec_scopes)? {
        PackedNode::Composite {
            preprocessed_root, ..
        } => preprocessed_root
            .clone()
            .try_into()
            .map_err(|root: Vec<u32>| {
                HintError::CustomHint(
                    format!(
                        "Packed node root has {} words; expected {BLAKE2S_DIGEST_N_WORDS}.",
                        root.len()
                    )
                    .into(),
                )
    ...
    let node_root_felts = node_root.map(Felt252::from);
    ...
    for index in 0..n_supported_roots {
        let root_ptr = (supported_roots_ptr + index * BLAKE2S_DIGEST_N_WORDS)?;
        let words = vm.get_integer_range(root_ptr, BLAKE2S_DIGEST_N_WORDS)?;
        if words
            .iter()
            .zip(&node_root_felts)
...
pub fn circuit_unpack_set_leaf_data(
    ...
    let PackedNode::Plain { output_preimage } = &composite_subtasks(node)?[0] else {
        return Err(HintError::CustomHint(
            "Leaf subtask is not a Plain preimage reveal.".into(),
        ));
    };

    let preimage: Vec<Felt252> = output_preimage
        .iter()
        .map(|s| felt_from_decimal_str(s))
        .collect::<Result<_, _>>()?;
...
mod tests {
    ...
    #[test]
    fn test_set_root_index_rejects_malformed_node_root() {
        let (mut vm, ids_data, ap_tracking) = vm_with_ids(&["config", "root_index"]);
        load_config(&mut vm, &ids_data, &ap_tracking, &[sample_root(10)]);
        // A short node root must be rejected, not prefix-matched against the supported list.
        let mut exec_scopes = scopes_with_node(PackedNode::Composite {
            preprocessed_root: sample_root(10)[..3].to_vec(),
    ...
    fn test_set_leaf_data_rejects_malformed_leaves() {
        ...
        let mut exec_scopes = scopes_with_node(PackedNode::Composite {
            ...
            subtasks: vec![PackedNode::Plain {
                output_preimage: vec!["0xabc".to_string()],
            }],
        });
        assert!(
            circuit_unpack_set_leaf_data(&mut vm, &mut exec_scopes, &ids_data, &ap_tracking)
    ...
    fn test_load_mock_circuit_verifier_input_requires_ids() {
        ...
        for present_ids in [
            &[][..],
            &["preprocessed_root"][..],
            &["preprocessed_root", "output_values"][..],
        ] {
    ...
    #[test]
    fn test_set_root_index_rejects_non_composite_node() {
        let (mut vm, ids_data, ap_tracking) = vm_with_ids(&["config", "root_index"]);
        load_config(&mut vm, &ids_data, &ap_tracking, &[sample_root(10)]);
        let mut exec_scopes = scopes_with_node(PackedNode::Plain {
            output_preimage: vec![],
    ...
    fn test_setup_hints_require_ids() {
        ...
        exec_scopes.insert_value(
            PROGRAM_INPUT,
            ProgramInput::Json(serde_json::json!({}).to_string()),
        );
    ...
    fn test_set_leaf_data_requires_ids() {
        ...
        let (mut vm, ids_data, ap_tracking) = vm_with_ids(&["h1_low", "h1_high", "preimage"]);
        let mut exec_scopes = scopes_with_node(leaf_node(20));
        assert!(
            circuit_unpack_set_leaf_data(&mut vm, &mut exec_scopes, &ids_data, &ap_tracking)
                .is_err()
