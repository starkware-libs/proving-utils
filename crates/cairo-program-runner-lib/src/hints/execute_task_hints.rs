...
pub fn determine_use_prev_hash(
    ...
    let prev_program_data_length = vm.get_integer(prev_program_data_length_ptr).map(|cow| cow.into_owned())?;
    ...
    let prev_program: Vec<Felt252> = vm
        ...
        .get_integer_range(
            prev_program_main_ptr,
                felt_to_usize(&prev_program_data_length).unwrap()).unwrap().iter()
    ...
    // compare the programs' sizes
    if prev_program.len() != program_from_task.data.len() {
    ...
    //compare the opcodes
    for (index, opcode) in program_from_task.data.iter().enumerate() {
        if prev_program[index] != field_element_to_felt(maybe_relocatable_to_field_element(opcode).unwrap()) {
            insert_value_from_var_name(
                "use_prev_hash",
                0,
                vm,
