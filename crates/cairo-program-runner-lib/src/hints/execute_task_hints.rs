    ...
    for (index, builtin) in ALL_BUILTINS.iter().enumerate() {
        if used_builtins.contains(builtin) {
            ...
            if simulated_builtins.contains(builtin) {
                ...
                // The builtin was simulated, hence the ptr wasn't updated (as we passed a null ptt
                // to the subtask), and so we restore the value from the pre-execution builtin ptrs.
