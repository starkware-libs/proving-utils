...
pub fn append_fact_topologies(
    ...
    // Get Arc<Task> here (cheap clone) since we need mutable access to exec_scopes for
    // fact_topologies.
...
pub fn setup_subtask_for_execution(
    ...
    // Get Arc<Task> (cheap clone) since we need mutable access to exec_scopes throughout this
    // function.
    let task: Arc<Task> = exec_scopes.get(vars::TASK)?;
