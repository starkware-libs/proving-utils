...
mod test {
    ...
    fn test_divide_queries_ind_by_coset_size_to_fp_offset(
        ...
        let fp  = vm.get_fp();
        vm.insert_value(Relocatable::from((fp.segment_index, fp.offset - 2)), MaybeRelocatable::from(queries_addr))
        .expect("Failed to insert queries pointer");
        vm.insert_value(Relocatable::from((fp.segment_index, fp.offset - 1)), MaybeRelocatable::from(params_addr))
