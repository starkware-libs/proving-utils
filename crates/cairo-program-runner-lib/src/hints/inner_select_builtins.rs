...
///         and memory[ids.all_encodings] == memory[ids.task_simulated_builtins_encodings]
///     )
...
pub fn select_builtin(
    ...
    let (select_builtin, is_simulated_builtin) = if n_selected_builtins == 0 {
        ...
        let is_simulated_builtin = if n_simulated_builtins_left == 0 {
            ...
            let task_simulated_builtins_encodings = get_ptr_from_var_name(
                "task_simulated_builtins_encodings",
                vm,
                ids_data,
                ap_tracking,
            )?;
            let simulated_builtin_encoding = vm
                .get_integer(task_simulated_builtins_encodings)?
                .into_owned();
            builtin_encoding == simulated_builtin_encoding
        };
