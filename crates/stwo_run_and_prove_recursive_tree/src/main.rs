...
//!   `--leaves_input` (in), `--verifier_program` (in), `--bootloader_program` (in),
//!   `--proof_path` / `--program_output` / `--fact_topologies_path` / `--packed_output_path`
...
use stwo_run_and_prove_recursive_tree::{
    LeafInput, RecursiveTreeConfig, RecursiveTreeError, StwoProverEntryPoint,
...
struct Args {
    ...
    /// decompressed proof path, fact topology, and outputs.
    #[clap(long = "leaves_input")]
    leaves_input: PathBuf,
...
fn run() -> Result<(), RecursiveTreeError> {
    ...
    let args = Args::parse();
    let leaves = load_leaves(&args.leaves_input)?;
    info!(
        n_leaves = leaves.len(),
    ...
    let aggregated = stwo_run_and_prove_recursive_tree(config, &StwoProverEntryPoint)?;
    info!(
        n_non_recursive_jobs = aggregated.counters.n_non_recursive_jobs,
        total_n_pages = aggregated.counters.total_n_pages,
...
fn load_leaves(path: &PathBuf) -> Result<Vec<LeafInput>, RecursiveTreeError> {
    let content =
        fs::read_to_string(path).map_err(|e| RecursiveTreeError::PathIO(e, path.clone()))?;
