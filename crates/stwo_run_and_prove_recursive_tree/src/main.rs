...
struct Args {
    /// JSON file containing `{"leaves": [LeafInput, ...]}`. Each entry references a leaf's circuit
    /// proof path; the leaf's output values are read from the sidecar next to that proof. Named
