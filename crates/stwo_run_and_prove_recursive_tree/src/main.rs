...
use stwo_run_and_prove_recursive_tree::{
    LeafInput, RecursiveTreeConfig, RecursiveTreeError, stwo_run_and_prove_recursive_tree,
};
...
fn run() -> Result<(), RecursiveTreeError> {
    ...
    let aggregated = stwo_run_and_prove_recursive_tree(config)?;
    info!(
        n_non_recursive_jobs = aggregated.n_non_recursive_jobs,
        total_n_pages = aggregated.total_n_pages,
