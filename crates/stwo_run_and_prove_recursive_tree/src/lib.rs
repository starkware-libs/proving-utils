...
//! the previous layer's proofs as Cairo1Executable user_args. An odd entry at any layer is carried
...
/// Fields mirror the per-leaf data the legacy per-pair flow would have looked up from storage
...
pub struct LeafInput {
    ...
    /// Hash function the verifier program should use when hashing the verified program.
    ...
    /// root `PackedOutput` matches what the standard layered flow would have produced.
    ...
    /// entire sub-tree), so passing them here is the only way to get the correct aggregation.
...
impl RecursiveJobData {
    ...
    fn from_leaf(leaf: &LeafInput, outputs: Vec<Felt252>) -> Self {
        Self {
            outputs,
            n_non_recursive_jobs: leaf.n_non_recursive_jobs,
            total_non_recursive_output_size: leaf.total_non_recursive_output_size,
            total_n_pages: leaf.total_n_pages,
            total_fact_tree_structures_len: leaf.total_fact_tree_structures_len,
        }
    }
    ...
    fn combine(left: &Self, right: &Self, new_outputs: Vec<Felt252>) -> Self {
        Self {
            outputs: new_outputs,
            n_non_recursive_jobs: left.n_non_recursive_jobs + right.n_non_recursive_jobs,
            total_non_recursive_output_size: left.total_non_recursive_output_size
                + right.total_non_recursive_output_size,
            total_n_pages: left.total_n_pages + right.total_n_pages,
            total_fact_tree_structures_len: left.total_fact_tree_structures_len
                + right.total_fact_tree_structures_len,
        }
    }
}
...
pub struct RecursiveTreeConfig {
    ...
    /// If true, verify each layer's proof immediately after generation (matches the legacy
    /// stwo_run_and_prove `--verify` flag).
    pub verify: bool,
...
pub fn stwo_run_and_prove_recursive_tree(
    ...
    info!(
        n_leaves = leaves.len(),
        leaf_train_ids = ?leaves.iter().map(|l| l.train_id).collect::<Vec<_>>(),
    ...
    let mut current_layer: Vec<LayerEntry> = leaves
        ...
        .map(|leaf| {
            ...
            Ok(LayerEntry {
                ...
                aggregated: RecursiveJobData::from_leaf(&leaf, leaf_outputs.clone()),
    ...
    let root = current_layer
        .pop()
        .expect("layer.len() == 1 after the loop");
    write_root_outputs(
        &root,
        &proof_path,
        &program_output,
...
fn reduce_pair(
    ...
    debug_data_dir: Option<&std::path::Path>,
    ...
    if save_debug_data && let Some(dir) = debug_data_dir {
        // Mirror the existing stwo_run_and_prove debug behavior: dump the prover input under a
        // per-layer/pair filename so the artifacts don't collide between layers.
        let dump_path = dir.join(format!(
            "prover_input_layer_{layer_idx:03}_pair_{pair_idx:03}.json"
        ));
        fs::create_dir_all(dir)?;
        std::fs::write(&dump_path, sonic_rs::to_string_pretty(&prover_input)?)
            .map_err(|e| RecursiveTreeError::PathIO(e, dump_path))?;
    }

    let aggregated =
        RecursiveJobData::combine(&left.aggregated, &right.aggregated, outputs.clone());
...
/// Serde helper that serializes `Vec<Felt252>` as a JSON array of decimal strings. Decimal
/// preserves Felt252's full 252-bit precision through any JSON parser and is accepted by
/// Python's `marshmallow.fields.Integer.deserialize`, which can't handle hex or values outside
/// JSON's safe-integer range. Used by `RecursiveJobData.outputs` for the (Serialize-only) debug
/// dump from `main.rs`.
