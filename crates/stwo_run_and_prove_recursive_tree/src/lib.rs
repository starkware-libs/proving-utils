...
//! root node's two output values, and a nested `packed_output` JSON tree mirroring the fold (each
//! node carries its two output values and its subtasks) for a future unpacker.
...
pub enum RecursiveTreeError {
    ...
    Proving(String),
    #[error("Expected exactly 2 output values from a circuit proof, got {0}.")]
    ...
    PaddingParity,
    #[error("Could not parse output values for leaf train_id={train_id}: {reason}")]
    BadLeafOutputs { train_id: u64, reason: String },
}
