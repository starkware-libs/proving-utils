...
pub use leaf_io::{LeafProofExt, load_leaves};
...
pub struct RecursiveTreeConfig {
    ...
    pub leaves: Vec<SerializedLeafProof>,
    /// Output path for the root proof: the Cairo circuit verifier's felt252 arguments stream
    /// for a multi-leaf tree, or the leaf's serialized proof unchanged for a single-leaf tree.
