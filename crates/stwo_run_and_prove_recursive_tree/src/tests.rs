...
mod e2e {
    ...
    /// Preprocessed root of the privacy cairo-verifier circuit that produced the fixture proof —
    /// declared in each leaf's `LeafInput`. Matches
    /// `circuit_multiverifier::verify_test::PRIVACY_CAIRO_VERIFIER_PREPROCESSED_ROOT`.
    ...
    /// Preprocessed root of the multiverifier circuit (carried by every internal node). Matches
    /// `circuit_multiverifier::verify_test::MULTIVERIFIER_PREPROCESSED_ROOT`.
    const MULTIVERIFIER_PREPROCESSED_ROOT: [u32; circuit_common::N_RESERVED] = [
    ...
    fn expected_reduce(left: ExpectedNode, right: ExpectedNode) -> ExpectedNode {
        ...
        ExpectedNode {
            output_words,
            preprocessed_root: MULTIVERIFIER_PREPROCESSED_ROOT,
            packed: PackedNode {
    ...
    /// and returns the expected root node.
    fn expected_root(n: usize) -> ExpectedNode {
        ...
        while layer.len() > 1 {
            ...
            while let Some(left) = pairs.next() {
                match pairs.next() {
                    Some(right) => next.push(expected_reduce(left, right)),
                    None => next.push(left),
                }
            }
            layer = next;
