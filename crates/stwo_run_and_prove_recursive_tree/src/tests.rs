...
use crate::{LeafProofExt, RecursiveTreeError, SerializedLeafProof, fold_plan, load_leaves};
...
fn leaf_input_roundtrips() {
    ...
    // `output_preimage` is intentionally absent: the backend injects it, so it must serde-default
    // to empty on leaf files that don't carry it.
    let json = format!(
    ...
    let leaf: SerializedLeafProof = serde_json::from_str(&json).unwrap();
    assert_eq!(leaf.program_output, vec!["42", "7"]);
    assert!(leaf.output_preimage.is_empty());
    ...
    let leaf2: SerializedLeafProof = serde_json::from_str(&back).unwrap();
...
mod e2e {
    ...
    use crate::{RecursiveTreeConfig, SerializedLeafProof, stwo_run_and_prove_recursive_tree};
    ...
    fn dupe_and_fold(n: usize, dir: &std::path::Path) {
        ...
        let leaves: Vec<SerializedLeafProof> = (0..n)
            .map(|_| SerializedLeafProof {
                program_output: vec![],
                output_preimage: vec![],
