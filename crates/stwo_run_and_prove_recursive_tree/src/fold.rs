...
use crate::leaf_io::LeafProofExt;
...
impl LayerEntry {
    ...
    /// preprocessed root all travel inline in the [`SerializedLeafProof`].
    pub fn from_leaf(leaf: &SerializedLeafProof) -> Result<Self, RecursiveTreeError> {
        Ok(Self {
...
pub enum PackedNode {
    ...
    /// (each felt a decimal string; see `SerializedLeafProof::output_preimage`). Terminal — the
