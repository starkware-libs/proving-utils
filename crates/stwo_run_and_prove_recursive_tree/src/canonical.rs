...
fn leaf_cairo_verifier_config() -> CairoVerifierConfig {
    ...
    // The disabled set must track the preprocessed-trace variant, exactly as
    // `leaf_prover::prove_leaf` selects it — a leaf verifier shape built with the wrong set
    // would not match real leaf proofs.
