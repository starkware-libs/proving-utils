...
/// One line with the per-component non-padded row counts and padded log sizes for the given
/// verified trace log size.
fn format_sizes(trace_log_size: u32, raw: &RawSizes, padded: &ComponentSizes) -> String {
    format!(
        "trace_log_size = {trace_log_size}: eq={} (log: {}, {:.1}%), qm31_ops={} (log: {}, {:.1}%), \
         m31_to_u32={} (log: {}, {:.1}%), triple_xor={} (log: {}, {:.1}%), \
         blake_g_gate={} (log: {}, {:.1}%)",
        raw.eq,
        log_size(padded.eq),
        usage_percent(raw.eq),
        raw.qm31_ops,
        log_size(padded.qm31_ops),
        usage_percent(raw.qm31_ops),
        raw.m31_to_u32,
        log_size(padded.m31_to_u32),
        usage_percent(raw.m31_to_u32),
        raw.triple_xor,
        log_size(padded.triple_xor),
        usage_percent(raw.triple_xor),
        raw.blake_g_gate,
        log_size(padded.blake_g_gate),
        usage_percent(raw.blake_g_gate),
    )
}
