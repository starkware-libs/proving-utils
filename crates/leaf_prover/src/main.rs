...
fn run() -> Result<(), String> {
    ...
    let cairo_prover_parameters =
        sonic_rs::from_str(&read_to_string(&args.cairo_prover_params_json).unwrap()).unwrap();
    let circuit_prover_pcs_config =
        sonic_rs::from_str(&read_to_string(&args.circuit_prover_params_json).unwrap()).unwrap();
    let output = prove_leaf::prove_leaf(
        &program,
        program_input,
