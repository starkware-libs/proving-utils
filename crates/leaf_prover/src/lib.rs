...
use circuit_prover::prover::{prepare_circuit_proof_for_circuit_verifier, prove_circuit_assignment, BaseColumnPool};
...
pub fn prove_leaf(
    ...
    let n_outputs = *runner.get_execution_resources().unwrap().builtin_instance_counter.get(&BuiltinName::output).unwrap_or(&0);
    ...
    let (proof_for_circuit, serialized_proof_header) =
    ...
    fs::write(&proof_path, proof_bytes).unwrap_or_else(|err| panic!("Cannot write proof to {}. Error: {err}", proof_path.display()))
}
