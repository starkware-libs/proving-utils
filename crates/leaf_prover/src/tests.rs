...
use crate::{consts::{CIRCUIT_PCS_CONFIG, CIRCUIT_TRACE_LOG_SIZE}, prove_leaf};
...
fn test_prove_leaf_small_program_canonical_small() {
    ...
    let prover_parameters = sonic_rs::from_str(
        &read_to_string(project_dir.join("test_data/prover_params_canonical_small.json")).unwrap(),
    )
    .unwrap();
    ...
    pcs_config.lifting_log_size = Some(CIRCUIT_TRACE_LOG_SIZE + pcs_config.fri_config.log_blowup_factor);
    ...
    if std::env::var("FIX").is_ok() {
        ...
        .unwrap_or_else(|err| panic!("Error writing to {}: {err}", expected_output_path.display()));
    } else {
