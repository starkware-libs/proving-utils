...
- The options for `--layout` are: `plain`, `small`, `dex`, `recursive`, `starknet`, `starknet_with_keccak`, `recursive_large_output`, `recursive_with_poseidon`, `all_solidity`, `all_cairo`, `dynamic`, `stwo_no_ecop`, `perpetual`, `dex_with_bitwise`. See the `LayoutName` enum definition in `cairo-vm`.
