...
-> Result<Arc<RecursiveProverPrecomputes>, Box<dyn Error>> {
    ...
    assert_ne!(
        cairo_lifting_log_size, 0,
        "min_lifting_log_size is not set in Cairo's PcsConfig"
    );
    assert_ne!(
        circuit_lifting_log_size, 0,
        "min_lifting_log_size is not set in Circuit's PcsConfig"
    );
