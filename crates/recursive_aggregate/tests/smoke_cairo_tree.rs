...
use recursive_aggregate::{
    AggregateConfig, CircuitPrecompute, FOLD_ARITY, PoolSet, TreeProof, ZkBlind,
    node_preprocessed_from_shared, preprocessed_root, prove_root_verification,
    recursive_aggregate_prove,
};
...
fn aggregate_config() -> AggregateConfig {
    ...
    AggregateConfig {
        ...
        leaf_preprocessed_root: HashValue::from(PRIVACY_CAIRO_VERIFIER_PREPROCESSED_ROOT),
        node_target_padding_sizes: target_padding_sizes(),
        leaf_target_padding_sizes: target_padding_sizes(),
        // Cairo stand-in: leaves share the node's shape (single-target regime), so the leaf and
        // node PCS coincide here.
...
fn load_cairo_leaves(config: &AggregateConfig, n: usize) -> Vec<TreeProof> {
    ...
    let proof =
        deserialize_proof_with_config(&mut bytes.as_slice(), &config.leaf_shared_config.proof_config)
