...
pub(crate) fn derive_unpacker_config(
    ...
    // For the NoValue recompute only the circuit STRUCTURE matters (NoValue carries no witness);
    // the concrete output words are inert, so zeros suffice.
    let root_output_values = [0u32; N_RESERVED];
    let leaf_output_values = vec![[0u32; N_RESERVED]; n];
...
fn trace_of(cfg: &CircuitConfig, blowup: u32) -> u32 {
    cfg.config.min_lifting_log_size - blowup
}
