...
pub struct TreeProof {
    ...
    /// The circuit's `N_RESERVED` output words — the unreduced Blake2s digest, one raw `u32` per
    /// word (the multiverifier hashes these one word per output; see `build_multiverifier_circuit`).
...
impl NodeKind {
    ...
    /// The child-verifier `SharedConfig` a node of `height` (above the leaves) uses to verify its
    /// children — the config that determines each child's in-circuit `circuit_hash`. Exposed to the
    /// unpacker so its reconstructed fold preimage matches the multiverifier's.
    pub(crate) fn child_shared_config(height: usize, config: &AggregateConfig) -> &SharedConfig {
        Self::from_height(height).shared_config(config)
    }
...
fn prove_leaf_or_short(
    ...
    let output_values: [u32; N_RESERVED] = unpack_output_values(&public_data.output_values)
...
/// Converts a proven circuit's `public_data.output_values` (each a `pack_u32`-embedded digest word,
/// i.e. QM31 `(low16, high16, 0, 0)`) into the raw `[u32; N_RESERVED]` the multiverifier hashes one
/// word per output. `unpack_u32` is exact (total for these values). `Err` if the count isn't
/// `N_RESERVED`.
pub(crate) fn unpack_output_values(values: &[QM31]) -> Result<[u32; N_RESERVED], Vec<u32>> {
    values.iter().map(|q| q.unpack_u32()).collect::<Vec<_>>().try_into()
}
